...
func _input(event: InputEvent) -> void:
	if !GlobalData.complete[0] and GlobalData.TutComplete: #클리어되지 않았을때만 작동
		if GlobalData.mt[0]!=1: #음뭐더라 그 범위 안에서 마우스 없으면 지워지게끔 바로
			clear_all_lines()
		...
		if event is InputEventMouseButton:
			if event.button_index == BUTTON_LEFT:
				...
				if _pressed and GlobalData.spot[0]==1:
					...
					if GlobalData.spot[0]!=2: # 마우스 놓을때 끝지점이 아니면 지워
						clear_all_lines()
					elif GlobalData.StartCurrent[0] and !GlobalData.complete[0]: #완료
